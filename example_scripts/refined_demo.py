...
refined_wikipedia = Refined.from_pretrained(model_name='wikipedia_model_with_numbers',
                                            entity_set="wikipedia")

refined_wikidata = Refined.from_pretrained(model_name='wikipedia_model_with_numbers',
                                           entity_set="wikidata")

#results_numbers = eval_all(refined=refined_wikidata, el=False)


# Difficult disambiguation example
text = 'Michael Jordan is a Professor of Computer Science at UC Berkeley.'
spans = refined_wikidata.process_text(text)
print('\n' + '****' * 10 + '\n')
print(text)
print(spans)
print('\n' + '****' * 10 + '\n')
...
text = 'Michael Jordan is a Professor of Computer Science at UC Berkeley.'
spans = refined_wikipedia.process_text(text)
print('\n' + '****' * 10 + '\n')
print(text)
print(spans)
print('\n' + '****' * 10 + '\n')
...
spans = refined_wikidata.process_text(text, spans=[Span(text='Joe Biden', start=0, ln=10),
...
docs = refined_wikidata.process_text_batch(texts)
for doc in docs:
...
docs = refined_wikidata.process_text_batch(texts=texts, spanss=spanss)
for doc in docs:
    print(f'Document: {doc.text}, spans: {doc.spans}')
print('\n' + '****' * 10 + '\n')
